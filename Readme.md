...
- 应对海量数据，分散存储压力 —— 数据库分片（单表500万条 or 2GB）
...
CAP原理（布鲁尔定理）是分布式系统的**基础理论**，它指出分布式数据存储系统**无法同时满足**一致性、可用性和分区容错性这三项要求，最多只能同时满足其中两项。

- **C（Consistency，一致性）**：所有节点在同一时刻看到的数据完全相同。更严格地说，**读操作必须返回最新的写入结果**（强一致性）。如果数据还没同步完，系统会拒绝响应，直到数据一致。
- **A（Availability，可用性）**：每次请求都能收到**非错误的响应**（成功或失败都算），但不能是超时或无响应。系统必须“随时在线”，保证每个节点都可用。
- **P（Partition Tolerance，分区容错性）**：集群中部分节点因网络故障、丢包等原因无法通信（形成“网络分区”）时，系统整体仍能继续运行。

**最大的误区：不是“三选二”，在分布式系统中，P（分区容错）是必须满足的硬性条件**。因为网络硬件必然会出现故障，你不选P，一旦分区发生，系统就彻底瘫痪了。

所以，**CAP定理本质是在发生网络分区时，让你在 C 和 A 之间二选一**：

- **放弃 A（选择 CP）**：发生分区时，为保证数据一致，系统暂停服务，返回错误或超时。**典型代表**：银行转账系统、ZooKeeper、HBase、MongoDB。
- **放弃 C（选择 AP）**：发生分区时，为保证系统可用，节点继续响应请求，但数据可能暂时不一致（允许“脏读”），待分区恢复后再同步。**典型代表**：购物网站库存展示、Cassandra、DynamoDB、Riak。

CP案例：当发生分区时（例如写库下线），业务访问读库时，会返回error，因为无法知晓写库是否一致所以为了保证一致性，返回error

AP案例：当发生分区时（例如写库下线），业务访问读库时，会直接返回读库中的数据
...
- **L（Latency延迟性）**：响应时间快
- **L** 与 **A** 的区别：**可用性（A）看的是“有没有响应”，延迟（L）看的是“响应有多快”**。

现实中的数据库取舍，更常用 **PACELC** 理论来补充：

- 如果发生**分区（P）**，在 A 和 C 之间选；
- 正常情况，在 L 和 C 之间选 
  - 保证 C：MySQL 主从同步时需等待从库确认，这会增加延迟
  - 保证 L ：而为了低延迟，很多 NoSQL 允许异步复制，牺牲短暂的一致性。



| **分类**  | **发生分区时 (P)**           | **正常运行时 (Else)**          | **典型数据库代表**                                           |
| --------- | ---------------------------- | ------------------------------ | ------------------------------------------------------------ |
| **PA/EC** | 放弃一致性，选 **A**（可用） | 放弃延迟，选 **C**（强一致）   | **MongoDB**（默认配置）、**DynamoDB**（可配置强一致读）      |
| **PA/EL** | 放弃一致性，选 **A**（可用） | 放弃一致性，选 **L**（低延迟） | **Cassandra**、**Riak**、**DynamoDB**（默认最终一致性）      |
| **PC/EC** | 放弃可用性，选 **C**（一致） | 放弃延迟，选 **C**（强一致）   | **HBase**、**Spanner**（Google）、**CockroachDB**            |
| **PC/EL** | 放弃可用性，选 **C**（一致） | 放弃一致性，选 **L**（低延迟） | 极少见（逻辑矛盾：分区时宁可停服也要一致，正常时却允许脏读，这种设计通常不合理） |
...
ShardingSphere-JDBC使用时，需要在项目中配置ShardingSphere，数据源交给ShardingSphere管理，应用无感对接逻辑库表。

ShardingSphere-Proxy在使用时，在中间件中配置ShardingSphere，应用像连接数据库一样连接中间件即可（代理层）
...
```shell
vi /atguigu/mysql/master/conf/my.cnf
```
...
#进入容器内的mysql命令行
mysql -uroot -p
#修改密码校验方式（mysql8.0的密码caching_sha2_password比较新，建议修改成mysql_native_password）
...
```sql
CHANGE MASTER TO MASTER_HOST='192.168.56.10', 
MASTER_USER='atguigu_slave',MASTER_PASSWORD='123456', MASTER_PORT=3310,
...
```
# 仅修改端口操作
CHANGE MASTER TO MASTER_PORT = 3310;
```
...
shardingsphere存在两种模式：单机模式\集群模式 (推荐)
区别是集群模式会将数据库元数据存储到注册中心
...
被事务封装的写后读，读操作会发给master，但是没有事务会发给slave
...
# 逻辑数据源指向myds，并不关心真正的主库
Logic SQL: INSERT INTO t_user  ( uname )  VALUES  ( ? )
# 真实写入的库由shardingsphere配置决定
Actual SQL: server-master ::: INSERT INTO t_user  ( uname )  VALUES  ( ? ) ::: [张三丰]


Actual SQL: server-slave0 ::: SELECT  id,uname  FROM t_user
[User(id=1, uname=zhang3), User(id=2, uname=0c334c5a0b51), User(id=3, uname=li4), User(id=4, uname=张三丰), User(id=5, uname=铁锤)]
```
...
注意：垂直分表不需要ShardingSphere参与，直接人为分表即可，关联查询即可

这里只涉及垂直分库，ShardingSphere可以将多个数据库聚合对外为一个数据库
...
```
雪花算法：时间戳（41位） + 工作机器ID（10位） + 序列号（12位）
优点：有序数字类型对 B+Tree 索引更加友好

风险点（致命缺陷）：

时钟回拨（Clock Backward）：如果服务器开启了 NTP 时间同步，且发生时间跳变（回拨），那么新生成的时间戳可能会小于之前生成的时间戳，导致必定的重复 ID。
机器ID分配：如果运维手动配置了两个相同的 Worker ID，两台机器就会产生完全相同的 ID。

结论：只要代码中实现了时钟回拨补偿机制（如等待或换号），并且确保 Worker ID 全局唯一，雪花算法生成的 ID 就是绝对唯一的。
```

![1783516144771](/1783516144771.png)
...
```shell
mysql -h192.168.100.105 -P3307 -uroot -p
```
